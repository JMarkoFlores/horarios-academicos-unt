...
async function seedHorariosCicloIII() {
  ...
  const horariosCicloIIIData = [
    ...
    // 8
    { docente: "Sheyla Laura Escobedo Rodriguez", curso: "Psicologia Organizacional (e )", dia: "Martes", horas: "6-8", tipo: "Teoría/Práctica", ambiente: "posgrado A-311", grupo: 1 },
    { docente: "Sheyla Laura Escobedo Rodriguez", curso: "Psicologia Organizacional (e )", dia: "Viernes", horas: "6-8", tipo: "Teoría/Práctica", ambiente: "posgrado A-311", grupo: 1 },
  ];
  ...
  for (const data of horariosCicloIIIData) {
    const docente = dbDocentes.find(d => {
      const fullNombre = `${d.nombres} ${d.apellidos}`.toLowerCase();
      const searchNombre = data.docente.toLowerCase();
      return fullNombre.includes(searchNombre) || searchNombre.includes(fullNombre);
    });
    
    const curso = dbCursos.find(c => {
      const dbNombre = c.nombre.toLowerCase();
      const searchNombre = data.curso.toLowerCase();
      // Limpiar un poco el nombre de búsqueda (quitar el (e) o similares)
