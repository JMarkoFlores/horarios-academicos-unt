...
async function seed() {
  ...
  // ── 4. AMBIENTES (Aulas y Laboratorios) ──────────────────────────────
  ...
  // ── 12. ASIGNACIÓN DE HORARIOS (CICLO I) ──────────────────────────────
  console.log("📅 Creando asignaciones de horarios para Ciclo I...");
  const horariosSeedData = [
    {
    ...
    },
    {
      ...
      fin: "13:00:00",
      tipo: TipoClase.TEORIA,
      ambienteCodigo: "TALLER-CONFECCIONES",
    },
    {
  ...
  for (const hData of horariosSeedData) {
    const docente = dbDocentes.find((d) => d.codigo === hData.docenteCodigo);
    const curso = dbCursos.find((c) => c.codigo === hData.cursoCodigo);
    const ambiente = dbAmbientes.find((a) => a.codigo === hData.ambienteCodigo);
    const grupo = await grupoRepo.findOne({
      where: {
        codigo: `${hData.cursoCodigo}-G${hData.grupoNum}`,
        periodo_academico: { id: periodoId },
      },
    });

    if (docente && curso && ambiente && grupo) {
      await horarioRepo.save(
        horarioRepo.create({
          ...
          periodo: "2026-I",
          dia: hData.dia,
          hora_inicio: hData.inicio,
          hora_fin: hData.fin,
          tipo_clase: hData.tipo,
  ...
  console.log("✅ Asignaciones de horarios de Ciclo I creadas\n");

  // ── 13. FACULTADES, ESCUELAS Y DEPARTAMENTOS ────────────────────────────────────
