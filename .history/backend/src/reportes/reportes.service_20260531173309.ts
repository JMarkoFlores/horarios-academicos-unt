...
export class ReportesService {
  ...
  async generarReporteTodosCiclosPDF(periodo: string): Promise<Buffer> {
    ...
    doc.setFillColor(26, 35, 126); // primary
    doc.rect(0, 0, PAGE_W, 40, 'F');
    
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(20);
    doc.text('PROGRAMACIÓN ACADÉMICA', PAGE_W / 2, 20, { align: 'center' });
    doc.setFontSize(14);
    doc.text(`SEMESTRE ACADÉMICO ${periodo}`, PAGE_W / 2, 30, { align: 'center' });
    ...
    currentY += 10;
    doc.setFont('helvetica', 'normal');
    doc.text('Universidad Nacional de Trujillo', 25, currentY); currentY += 7;
    ...
    for (const ciclo of ciclos) {
      ...
      await this.dibujarPaginaCiclo(doc, ciclo, periodo);
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Página ${i} de ${pageCount}`, pW - 20, pH - 10);
      doc.text(`Generado el: ${new Date().toLocaleString()}`, 20, pH - 10);
    }

    return Buffer.from(doc.output('arraybuffer'));
  }

  private async dibujarPaginaCiclo(doc: jsPDF, ciclo: number, periodo: string): Promise<void> {
    const horarios = await this.horarioRepo
      .createQueryBuilder("horario")
      .leftJoinAndSelect("horario.docente", "docente")
      .leftJoinAndSelect("horario.curso", "curso")
    ...
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text(`Periodo: ${periodo}`, PAGE_W - 12, 7, { align: 'right' });
    ...
    horarios.forEach(a => {
      ...
      const key = `${a.docente.id}-${a.curso.id}`;
      if (!profesoresCursosMap.has(key)) {
        profesoresCursosMap.set(key, { docente: a.docente, curso: a.curso, horas: 0 });
      }
      const entry = profesoresCursosMap.get(key);
      const hInicio = parseInt(a.hora_inicio.split(':')[0], 10);
      const hFin = parseInt(a.hora_fin.split(':')[0], 10);
    ...
    const profesorColors: [number, number, number][] = [
      [255, 205, 210], [248, 187, 217], [240, 178, 122], [249, 231, 159], [213, 245, 227],
      [174, 214, 241], [215, 189, 226], [250, 219, 216], [169, 223, 191], [249, 235, 234]
    ];
    const profesorColorMap = new Map<number, [number, number, number]>();
    const profesoresUnicos = Array.from(new Set(horarios.map(h => h.docente?.id).filter(Boolean)));
    profesoresUnicos.forEach((id, idx) => profesorColorMap.set(id, profesorColors[idx % profesorColors.length]));

    autoTable(doc, {
      startY: yPos,
      head: [['N°', 'Profesor', 'Asignatura', 'T', 'P', 'L', 'T. Horas']],
      body: profesoresCursos.map((item, idx) => [
        idx + 1, `${item.docente.apellidos}, ${item.docente.nombres}`, item.curso.nombre,
        item.curso.horas_teoria || '-', '-', item.curso.horas_laboratorio || '-', item.horas
      ]),
      theme: 'grid',
      styles: { fontSize: 6, cellPadding: 1.5, valign: 'middle', lineColor: C.border, lineWidth: 0.25 },
      headStyles: { fillColor: C.primary, textColor: C.white, fontStyle: 'bold', fontSize: 7, halign: 'center' },
      didParseCell: (data) => {
        if (data.section === 'body' && data.column.index === 1) {
          const item = profesoresCursos[data.row.index];
          if (item?.docente?.id) data.cell.styles.fillColor = profesorColorMap.get(item.docente.id) ?? [255, 255, 255];
        }
      },
      margin: { left: 8, right: 8 }
    });
    ...
    const dias = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
    const diasNum = [1, 2, 3, 4, 5, 6];
    const horas = Array.from({ length: 18 }, (_, i) => i + 7);
    const cellHeight = 7.5;
    const cellWidth = (PAGE_W - 16 - 20) / 7;
    const horaColWidth = 20;
    const gridWidth = PAGE_W - 16;
    ...
    doc.setFillColor(79, 70, 229);
    doc.rect(8, yPos, gridWidth, 10, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(9);
    doc.text('HORA', 10, yPos + 7);
    dias.forEach((dia, idx) => doc.text(dia, 8 + horaColWidth + idx * cellWidth + cellWidth / 2, yPos + 7, { align: 'center' }));
    ...
    horarios.forEach(h => {
      ...
      const rowStart = horas.indexOf(hIni);
      const rowEnd = horas.indexOf(hFin);
      if (rowStart === -1 || rowEnd === -1) return;

      const blockX = 8 + horaColWidth + diaIdx * cellWidth;
      const blockY = yPos + 10 + rowStart * cellHeight;
      const blockH = (rowEnd - rowStart) * cellHeight;

      doc.setFillColor(...(profesorColorMap.get(h.docente?.id) ?? [255, 255, 255]));
      doc.rect(blockX, blockY, cellWidth, blockH, 'F');
      doc.setDrawColor(150, 150, 150);
      doc.rect(blockX, blockY, cellWidth, blockH, 'S');
      doc.setTextColor(51, 51, 51);
      doc.setFontSize(4);
      doc.text(h.docente?.apellidos || '', blockX + cellWidth / 2, blockY + 2.5, { align: 'center' });
      doc.text(h.curso?.nombre || '', blockX + cellWidth / 2, blockY + 5, { align: 'center' });
      doc.text(h.ambiente?.codigo || '', blockX + cellWidth / 2, blockY + 7.5, { align: 'center' });
    });
  }
