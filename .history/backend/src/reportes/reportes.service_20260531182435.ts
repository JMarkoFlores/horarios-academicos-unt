...
export class ReportesService {
  ...
  async generarReporteTodosCiclosPDF(periodo: string): Promise<Buffer> {
    ...
    for (const ciclo of ciclos) {
      ...
      await this.dibujarPaginaCiclo(doc, ciclo, periodo, logoUrl, primaryRGB);
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Generado el: ${new Date().toLocaleString()}`, 15, pH - 8);
      doc.text(`Página ${i} de ${pageCount}`, pW - 15, pH - 8, { align: 'right' });
    }

    return Buffer.from(doc.output('arraybuffer'));
  }

  private async dibujarPaginaCiclo(doc: jsPDF, ciclo: number, periodo: string, logoUrl: string, primaryRGB: [number, number, number]): Promise<void> {
    const horarios = await this.horarioRepo
      .createQueryBuilder("horario")
      .leftJoinAndSelect("horario.docente", "docente")
      .leftJoinAndSelect("horario.curso", "curso")
    ...
    try {
      doc.addImage(logoUrl, 'PNG', 10, 3, 18, 18);
    } catch (e) {}
    ...
    const cellHeight = 7; // Altura reducida para que quepa todo
    ...
    horarios.forEach(h => {
      ...
      if (rowStart !== -1 && rowEnd !== -1) {
        const blockX = 10 + horaColWidth + diaIdx * cellWidth + 0.5;
        const blockY = yPos + 8 + rowStart * cellHeight + 0.5;
        const blockW = cellWidth - 1;
        ...
        const splitCurso = doc.splitTextToSize(cursoNombre, blockW - 2);
        doc.text(splitCurso.slice(0, 2), blockX + blockW / 2, blockY + 3, { align: 'center' });
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(5);
        doc.text(docenteApellidos, blockX + blockW / 2, blockY + blockH - 5, { align: 'center' });
        
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...C.primary);
        doc.text(ambienteCodigo, blockX + blockW / 2, blockY + blockH - 2, { align: 'center' });
      }
    });
  }
