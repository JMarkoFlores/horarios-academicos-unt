...
export class ReportesService {
  ...
  async generarReporteDocentePDF(
    ...
    asignacionesPorDia.forEach((asigs, dia) => {
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(asig => {
          bloques.push({
            dia,
            horaInicio: horaToDecimal(asig.hora_inicio),
            horaFin: horaToDecimal(asig.hora_fin),
            asignacion: asig,
    ...
    bloques.forEach(bloque => {
      ...
      let labelSuffix = '';
      const dur = bloque.horaFin - bloque.horaInicio;
      if (bloque.asignacion.tipo_clase === TipoClase.TEORIA) labelSuffix = ` (${dur}T)`;
      else if (bloque.asignacion.tipo_clase === TipoClase.PRACTICA) labelSuffix = ` (${dur}P)`;
      else if (bloque.asignacion.tipo_clase === TipoClase.LABORATORIO) {
        const matches = bloque.asignacion.grupo?.codigo?.match(/-G(\d+)$/);
        const gNum = matches ? matches[1] : (bloque.asignacion.grupo?.codigo || '');
        labelSuffix = ` (${dur}L-G${gNum})`;
      }

      const curso = (bloque.asignacion.curso?.nombre ?? '—') + labelSuffix;
      const amb = bloque.asignacion.ambiente?.codigo ?? '—';
      const texto1 = curso.length > 25 ? curso.substring(0, 25) + '...' : curso;
      const texto2 = amb;
    ...
    asignacionesPorDia.forEach((asigs, dia) => {
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(asig => {
          bloques.push({
            dia,
            horaInicio: horaToDecimal2(asig.hora_inicio),
            horaFin: horaToDecimal2(asig.hora_fin),
            asignacion: asig,
            left: carrilIdx * anchoPorBloque,
            width: anchoPorBloque
          });
  ...
  private async dibujarPaginaCiclo(doc: jsPDF, ciclo: number, periodo: string, logoBase64: string | null, primaryRGB: [number, number, number]): Promise<void> {
    ...
    autoTable(doc, {
      startY: yPos,
      head: [['N°', 'Profesor', 'Asignatura', 'T', 'P', 'L', 'Hrs']],
      ...
      theme: 'grid',
      styles: { fontSize: 7, cellPadding: 1, valign: 'middle', lineColor: [220, 220, 220], lineWidth: 0.1 },
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(h => {
          ...
          if (rowStart !== -1 && rowEnd !== -1) {
            ...
            const color = profesorColorMap.get(h.docente?.id) || [255, 255, 255];
            doc.setFillColor(...color);
            ...
            let labelSuffix = '';
            if (h.tipo_clase === TipoClase.TEORIA) labelSuffix = ' (T)';
            else if (h.tipo_clase === TipoClase.PRACTICA) labelSuffix = ' (P)';
            else if (h.tipo_clase === TipoClase.LABORATORIO) {
              const matches = h.grupo?.codigo?.match(/-G(\d+)$/);
              const gNum = matches ? matches[1] : (h.grupo?.codigo || '');
              labelSuffix = ` (L-G${gNum})`;
            }
            
            const cursoNombre = (h.curso?.nombre || '') + labelSuffix;
            const docenteApellidos = h.docente?.apellidos || '';
            const ambienteCodigo = h.ambiente?.codigo || '';
            ...
            if (blockH > 10) {
              ...
              doc.text(ambienteCodigo, blockX + blockW / 2, blockY + blockH - 1.5, { align: 'center' });
            }
          }
        });
      });
