...
export class ReportesService {
  ...
  async generarReporteDocentePDF(
    ...
    // Colores pasteles por profesor y curso
    const profesorCursoColorMap = new Map<string, [number, number, number]>();
    ...
    const getColorForProfesorCurso = (docenteIdParam: number | undefined, cursoIdParam: number | undefined): [number, number, number] => {
      if (!docenteIdParam || !cursoIdParam) return profesorColors[0];
      const key = `${docenteIdParam}-${cursoIdParam}`;
      if (!profesorCursoColorMap.has(key)) {
        const index = profesorCursoColorMap.size % profesorColors.length;
        profesorCursoColorMap.set(key, profesorColors[index]);
      }
      return profesorCursoColorMap.get(key)!;
    };
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(asig => {
          todosLosBloquesDelDia.push({
            asig,
            carrilIdx,
            hIni: horaToDecimal(asig.hora_inicio),
            hFin: horaToDecimal(asig.hora_fin)
          });
        });
      });
        ...
        bloquesEnCarril.forEach(asig => {
          ...
          if (fusionados.length > 0) {
            ...
            if (esTP && mismoAmbiente && ultimo.asignacion.curso?.id === asig.curso?.id && Math.abs(ultimo.horaFin - hIni) < 0.01) {
              ultimo.horaFin = hFin;
              ultimo.totalHoraFin = asig.hora_fin;
          ...
          fusionados.push({
            ...
            horaInicio: hIni,
            horaFin: hFin,
            totalHoraInicio: asig.hora_inicio,
            totalHoraFin: asig.hora_fin,
            asignacion: asig,
    ...
    bloques.forEach(bloque => {
      ...
      const color = getColorForProfesorCurso(bloque.asignacion.docente?.id, bloque.asignacion.curso?.id);
      ...
      const texto3 = `${bloque.totalHoraInicio}–${bloque.totalHoraFin}`;
      const textY = cellY + 4;
  ...
  async generarReporteCicloPDF(
    ...
    await this.dibujarPaginaCiclo(doc, ciclo, periodo, almuerzoInicio, almuerzoFin, C, PAGE_W, PAGE_H);
    ...
    const profesorCursoColorMapDoc = new Map<string, [number, number, number]>();
    profesoresCursos.forEach((item, idx) => {
      const key = `${item.docente.id}-${item.curso.id}`;
      profesorCursoColorMapDoc.set(key, profesorColorsDoc[idx % profesorColorsDoc.length]);
    });
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        ...
        if (item?.docente?.id && item?.curso?.id) {
          const key = `${item.docente.id}-${item.curso.id}`;
          const rgb = profesorCursoColorMapDoc.get(key);
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(asig => {
          todosLosBloquesDelDia.push({
            asig,
            carrilIdx,
            hIni: horaToDecimal2(asig.hora_inicio),
            hFin: horaToDecimal2(asig.hora_fin)
          });
        });
      });
    ...
    bloques.forEach(bloque => {
      ...
      const texto4 = `${bloque.totalHoraInicio.substring(0,5)}–${bloque.totalHoraFin.substring(0,5)}`;
      const textY = cellY + 2;
      doc.text(texto1, cellX + cellWidthBloque / 2, textY, { align: 'center' });
      ...
      doc.text(texto4, cellX + cellWidthBloque / 2, textY + 7.5, { align: 'center' });
    });
  ...
  private async dibujarCicloEnExcel(sheet: ExcelJS.Worksheet, ciclo: number, periodo: string): Promise<void> {
    const horarios = await this.horarioRepo
      .createQueryBuilder("horario")
      .leftJoinAndSelect("horario.docente", "docente")
      .leftJoinAndSelect("docente.departamento", "departamento")
    ...
    const rowColors = ['FFEBEE', 'FCE4EC', 'F3E5F5', 'EDE7F6', 'E8EAF6', 'E3F2FD', 'E0F7FA', 'E0F2F1', 'E8F5E9', 'F1F8E9'];
    ...
    profesoresCursos.forEach((item, idx) => {
      ...
      const row = [idx + 1, `${item.docente.apellidos}, ${item.docente.nombres}`, item.docente.departamento?.nombre || '—', item.curso.nombre, item.hTeoria || '-', item.hPractica || '-', item.hLaboratorio || '-', item.horas];
      row.forEach((val, i) => {
        const cell = sheet.getCell(currentRow, i + 1);
        cell.value = val;
    ...
    const gridHeaderRow = currentRow;
    currentRow++;
    ...
    horasArr.forEach(h => {
      ...
      const cellHora = sheet.getCell(`A${currentRow}`);
      cellHora.value = `${String(h).padStart(2, '0')}:00`;
      cellHora.font = { bold: true };
      cellHora.alignment = { horizontal: 'center' };
      ...
      for (let i = 2; i <= 7; i++) {
        ...
        cell.border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } };
        const isAlmuerzo = h >= almuerzoInicio && h < almuerzoFin;
        if (isAlmuerzo) {
          cell.value = 'ALMUERZO';
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8E1' } };
          cell.font = { color: { argb: 'B47800' }, size: 8 };
    ...
    // Dibujar bloques (Simplificado para Excel: sin carriles por ahora, solo el primer bloque)
    // Para Excel es complejo manejar múltiples bloques por celda, así que pondremos el contenido principal
    const sortedH = [...horarios].sort((a, b) => {
    ...
    sortedH.forEach(h => {
      const hIni = parseInt(h.hora_inicio.split(':')[0], 10);
      const hFin = parseInt(h.hora_fin.split(':')[0], 10);
      const diaIdx = h.dia || h.dia_semana;
      const startRow = horaToRowMap.get(hIni);
      const endRow = horaToRowMap.get(hFin - 1);

      if (startRow && endRow && diaIdx >= 1 && diaIdx <= 6) {
        const col = diaIdx + 1;
        const cell = sheet.getCell(startRow, col);
        
        // Si ya tiene valor (solapamiento), añadimos el texto
        const currentVal = cell.value;
        const labelPart = h.tipo_clase === TipoClase.TEORIA ? 'T' : h.tipo_clase === TipoClase.PRACTICA ? 'P' : 'L';
        const dur = hFin - hIni;
        const newText = `${h.curso?.nombre} (${dur}${labelPart})\n${h.docente?.apellidos}\n${h.ambiente?.codigo || ''}`;
        
        cell.value = currentVal ? `${currentVal}\n---\n${newText}` : newText;
        cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
        
        const colorKey = `${h.docente?.id}-${h.curso?.id}`;
