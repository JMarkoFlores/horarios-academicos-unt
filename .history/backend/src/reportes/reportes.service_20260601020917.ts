...
export class ReportesService {
  ...
  private async dibujarPaginaAmbiente(doc: jsPDF, ambienteId: number, periodo: string, logoBase64: string | null, primaryRGB: [number, number, number]): Promise<void> {
    ...
    horarios.forEach(h => {
      ...
      const startRowIdx = horas.indexOf(hIni);
      const endRowIdx = horas.indexOf(hFin);
      ...
      if (startRowIdx !== -1 && endRowIdx !== -1 && diaIdx >= 0 && diaIdx < 6) {
        ...
        const blockX = 10 + horaColWidth + diaIdx * cellWidth;

        const color = this.getColorForProfesorCurso(h.docente?.id, h.curso?.id);
        doc.setFillColor(...color);
        ...
        doc.setFontSize(6);
        doc.setFont('helvetica', 'bold');
        const cursoText = h.curso?.nombre || '';
        const docenteText = h.docente?.apellidos || '';
        const grupoText = h.grupo?.codigo || '';
        doc.text(cursoText.substring(0, 25), blockX + cellWidth / 2, blockY + 4, { align: 'center' });
        doc.text(docenteText.substring(0, 20), blockX + cellWidth / 2, blockY + 8, { align: 'center' });
        doc.text(grupoText, blockX + cellWidth / 2, blockY + 12, { align: 'center' });
      }
    });
  }
      ...
      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(h => {
          ...
          if (rowStart && rowEnd) {
            ...
            const labelPart = h.tipo_clase === TipoClase.TEORIA ? 'T' : 
                             h.tipo_clase === TipoClase.PRACTICA ? 'P' : 
                             `L-G${h.grupo?.codigo?.match(/-G(\d+)$/)?.[1] || ''}`;
            const dur = hFin - hIni;
            cell.value = `${h.curso?.nombre} (${dur}${labelPart})\n${h.docente?.apellidos}\n${h.ambiente?.nombre || h.ambiente?.codigo || ''}`;
