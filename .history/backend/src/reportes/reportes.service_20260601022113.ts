  ...
  async generarReporteDocentePDF(
    ...
    horarios.forEach(h => {
      ...
      if (startRowIdx !== -1 && endRowIdx !== -1 && diaIdx >= 0 && diaIdx < 6) {
        ...
        const blockX = 12 + horaColWidth + diaIdx * cellWidth;

        const color = this.getColorForProfesorCurso(docente.id, h.curso?.id);
        ...
        const ambienteText = h.ambiente?.codigo || '';
        const grupoText = h.grupo?.codigo || '';
        doc.text(cursoText.substring(0, 25), blockX + cellWidth / 2, blockY + 3, { align: 'center' });
        doc.text(ambienteText, blockX + cellWidth / 2, blockY + 6, { align: 'center' });
        if (blockH > 4) doc.text(grupoText, blockX + cellWidth / 2, blockY + 9, { align: 'center' });
      }
    });
