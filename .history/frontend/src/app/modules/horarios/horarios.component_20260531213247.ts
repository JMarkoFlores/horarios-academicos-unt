...
export class HorariosComponent implements OnInit, OnDestroy {
  ...
  private generarBloques(asignaciones: HorarioAsignado[]): any[] {
    ...
    asignacionesPorDia.forEach((asigs, dia) => {
      ...
      const widthPorBloque = 100 / (numCarriles || 1);

      carriles.forEach((bloquesEnCarril, carrilIdx) => {
        bloquesEnCarril.forEach(asig => {
          const horaInicio = this.horaToDecimal(asig.hora_inicio);
          const horaFin = this.horaToDecimal(asig.hora_fin);
          const dur = horaFin - horaInicio;
          
          let labelSuffix = '';
          if (asig.tipo_clase === 'TEORIA') labelSuffix = ` (${dur}T)`;
          else if (asig.tipo_clase === 'PRACTICA') labelSuffix = ` (${dur}P)`;
          else if (asig.tipo_clase === 'LABORATORIO') {
            const matches = asig.grupo?.codigo?.match(/-G(\d+)$/);
            const gNum = matches ? matches[1] : (asig.grupo?.codigo || '');
          ...
          bloquesFinales.push({
            ...
            label: (asig.curso?.nombre || '') + labelSuffix
          });
        });
      });
    });
  ...
  getProfesoresYCursos(): any[] {
    ...
    this.asignacionesCiclo.forEach(a => {
      ...
      else if (a.tipo_clase === 'LABORATORIO') entry.hLaboratorio += duration;
    });
