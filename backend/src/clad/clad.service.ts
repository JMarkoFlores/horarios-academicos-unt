...
export class CladService {
  ...
  async findAll(query: any, user: any) {
    ...
    if (query.periodo_academico_id) {
      qb.andWhere("c.periodo_academico_id = :periodo", {
        periodo: query.periodo_academico_id,
      });
    }
    if (query.estado) {
    ...
    if (user.rol === "docente") {
      ...
      qb.andWhere("d.departamento_id = :dptoId", {
        dptoId: user.departamento_id,
      });
  ...
  async create(dto: CreateCladDto, user: any) {
    ...
    const sumHoras = dto.detalles.reduce(
      (acc, curr) => acc + curr.horas_semanales,
      0,
    );

    const periodo = await this.cladRepo.manager.findOne(PeriodoAcademico, {
    ...
    const estadosActivos = [
      EstadoClad.BORRADOR,
      EstadoClad.ENVIADO_DPTO,
      EstadoClad.VALIDADO_DPTO,
      EstadoClad.VALIDADO_DEPENDENCIA,
      EstadoClad.APROBADO_FINAL,
    ];
    ...
    for (const det of dto.detalles) {
      for (const hor of det.horario) {
        const conflictos = await this.horarioRepo
          ...
          .andWhere("h.hora_inicio < CAST(:horaFin AS TIME)", {
            horaFin: hor.hora_fin,
          })
          .andWhere("h.hora_fin > CAST(:horaInicio AS TIME)", {
            horaInicio: hor.hora_inicio,
          })
          .getMany();

        if (conflictos.length > 0) {
          throw new BadRequestException(
            `Cruce de horario detectado en ${det.nombre_curso} (Día ${hor.dia} ${hor.hora_inicio}-${hor.hora_fin}) con un curso de Pregrado.`,
          );
        }
      }
    }
    ...
    for (const existing of existingClads) {
      for (const det of existing.detalles) {
        const horarios = Array.isArray(det.horario)
          ? (det.horario as any[])
          : [];
  ...
  async update(id: number, dto: CreateCladDto, user: any) {
    ...
    if (clad.docente_id !== user.docenteId)
      throw new ForbiddenException("No es el propietario");
    if (
      clad.estado !== EstadoClad.BORRADOR &&
  ...
  async remove(id: number, user: any) {
    const clad = await this.findOne(id);
    if (user.rol !== "admin" && clad.docente_id !== user.docenteId)
      throw new ForbiddenException();
    if (clad.estado !== EstadoClad.BORRADOR)
  ...
  async validarDpto(id: number, user: any) {
    ...
    if (
      user.rol !== "admin" &&
      clad.docente.departamento_id !== user.departamento_id
    )
      throw new ForbiddenException();
    if (clad.estado !== EstadoClad.ENVIADO_DPTO)
  ...
  async validarDependencia(id: number, user: any) {
    const clad = await this.findOne(id);
    if (clad.estado !== EstadoClad.VALIDADO_DPTO)
      throw new BadRequestException("Estado inválido");
  ...
  async aprobarFinal(id: number, user: any) {
    const clad = await this.findOne(id);
    if (user.rol !== "admin" && clad.docente.facultad_id !== user.facultad_id)
      throw new ForbiddenException();
    if (clad.estado !== EstadoClad.VALIDADO_DEPENDENCIA)
      throw new BadRequestException("Estado inválido");
