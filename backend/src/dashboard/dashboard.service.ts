...
export class DashboardService {
  ...
  async getKPIs(periodo: string) {
    ...
    const aulasOcupadas = new Set(
      horarios.filter((h) => h.ambiente?.tipo === TipoAmbiente.AULA).map((h) => h.ambiente?.id),
    ).size;
    const laboratoriosOcupados = new Set(
      horarios.filter((h) => h.ambiente?.tipo === TipoAmbiente.LABORATORIO).map((h) => h.ambiente?.id),
    ).size;
    ...
    const distribucionCategoria = [...new Set(docentes.map((d) => d.categoria))].map((cat) => {
    ...
    return {
      ...
      porcentaje_docentes_asignados:
        totalDocentes > 0 ? Math.round((docentesConHorario / totalDocentes) * 100) : 0,
      ...
      porcentaje_ocupacion_laboratorios:
        totalLaboratorios > 0 ? Math.round((laboratoriosOcupados / totalLaboratorios) * 100) : 0,
