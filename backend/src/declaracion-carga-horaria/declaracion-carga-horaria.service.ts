...
export class DeclaracionCargaHorariaService {
  ...
  async obtenerMia(usuario: Usuario & { docenteId?: number | null }): Promise<DeclaracionVista> {
    ...
    return {
      ...
      periodo,
      cargaLectiva,
      snapshotGuardado: this.normalizarSnapshotCargaLectiva(declaracion.carga_no_lectiva),
    };
  }
  ...
  async actualizar(
    ...
    declaracion.observaciones =
      dto.observaciones !== undefined ? dto.observaciones : declaracion.observaciones;
  ...
  async enviar(
    ...
    this.validarTransicionEstado(declaracion.estado, EstadoDeclaracionCarga.ENVIADO_DOCENTE);
  ...
  async validar(
    ...
    const declaracion = await this.obtenerEntidadEditable(id);
    this.validarTransicionEstado(declaracion.estado, EstadoDeclaracionCarga.VALIDADO_DPTO);
  ...
  async aprobar(
    ...
    this.verificarRol(usuario.rol, [RolUsuario.DECANO, RolUsuario.ADMINISTRADOR_SISTEMA]);
  ...
  validarTransicionEstado(actual: EstadoDeclaracionCarga, siguiente: EstadoDeclaracionCarga): void {
    ...
    const transiciones: Record<EstadoDeclaracionCarga, EstadoDeclaracionCarga[]> = {
      [EstadoDeclaracionCarga.NO_INICIADO]: [EstadoDeclaracionCarga.BORRADOR, EstadoDeclaracionCarga.ANULADO],
  ...
  private async obtenerEntidadEditable(id: number): Promise<DeclaracionCargaHoraria> {
    const declaracion = await this.declaracionRepo.findOne({
      where: { id },
      relations: [
        "docente",
  ...
  private esEstadoFinal(estado: EstadoDeclaracionCarga): boolean {
    return [EstadoDeclaracionCarga.CERRADO, EstadoDeclaracionCarga.ANULADO].includes(estado);
  }
  ...
  private async resolverDocente(
    ...
    if (docenteId) {
      const docente = await this.docenteRepo.findOne({
        where: { id: docenteId },
        relations: ["departamento", "departamento.escuela", "departamento.escuela.facultad", "facultad"],
      });
      if (!docente) {
  ...
  private verificarPermisoDocente(
    ...
    if (usuario.docenteId !== docenteId) {
      throw new ForbiddenException("No puede actuar sobre la declaración de otro docente");
    }
  }
  ...
  private verificarAccesoEdicion(
    ...
    if (usuario.rol !== RolUsuario.DOCENTE) {
      throw new ForbiddenException("No tiene permisos para editar la declaración");
    }

    if (usuario.docenteId !== docenteId) {
      throw new ForbiddenException("No puede editar la declaración de otro docente");
    }
  }

  private resolverEstadoObservacion(rol: RolUsuario): EstadoDeclaracionCarga {
    if (rol === RolUsuario.DIRECTOR_DEPARTAMENTO || rol === RolUsuario.ADMINISTRADOR_SISTEMA) {
    ...
    throw new ForbiddenException("Rol no autorizado para observar la declaración");
  }
  ...
  private async resolverVinculacionInstitucional(docente: Docente): Promise<{
    ...
    const facultad =
      docente.facultad ??
      (docente.facultad_id
        ? await this.facultadRepo.findOne({ where: { id: docente.facultad_id } })
        : null);

    const facultadId = facultad?.id ?? departamento?.escuela?.facultad?.id ?? null;
  ...
  private resolverSede(docente: Docente): string | null {
    return docente.facultad?.nombre ?? docente.departamento?.escuela?.facultad?.nombre ?? null;
  }
  ...
  async generarCargaLectivaDesdeHorarios(
    ...
    if (!periodo) {
      throw new NotFoundException(`Periodo académico ${periodoId} no encontrado`);
    }
    ...
    const horarios = await this.horarioRepo
      ...
      .andWhere("horario.estado = :estado", { estado: EstadoHorario.CONFIRMADO })
  ...
  async obtenerCargaLectivaDeclaracion(
    ...
    const declaracion = await this.obtenerDeclaracionConRelaciones(declaracionId);
  ...
  async actualizarCargaLectivaDeclaracion(
    ...
    declaracion.carga_no_lectiva = cargaLectiva as unknown as Record<string, unknown>;
    ...
    return {
      ...
      cargaLectiva,
      snapshotGuardado: this.normalizarSnapshotCargaLectiva(saved.carga_no_lectiva),
    };
  }

  private construirRegistroCargaLectiva(horario: HorarioAsignado): CargaLectivaDetalle {
    ...
    return {
      ...
      horasPractica,
      horasLaboratorio,
      horasBloque: this.calcularHorasBloque(horario.hora_inicio, horario.hora_fin),
  ...
  private construirResumenCargaLectiva(registros: CargaLectivaDetalle[]): ResumenCargaLectiva {
  ...
  private verificarAccesoDeclaracion(
    ...
    if (usuario.rol === RolUsuario.DOCENTE && usuario.docenteId !== declaracion.docente_id) {
      throw new ForbiddenException("No puede acceder a la declaración de otro docente");
    }
  }
