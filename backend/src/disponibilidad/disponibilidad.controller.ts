...
export class DisponibilidadController {
  ...
  async getByDocente(
    ...
    const result = await this.disponibilidadService.getByDocente(id, periodo ?? '');
    return { data: result, message: 'Disponibilidad obtenida correctamente' };
  }

  @Post('docente/:id')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Guardar disponibilidad masiva de un docente (reemplaza)' })
  @ApiParam({ name: 'id', type: Number })
  ...
  async guardarDisponibilidad(
    ...
    const result = await this.disponibilidadService.guardarDisponibilidadMasiva(id, dto);
  ...
  async getResumen(@Query('periodo') periodo: string) {
    const result = await this.disponibilidadService.getResumenDocentes(periodo ?? '');
