...
export class CargaNoLectivaController {
  ...
  @ApiOperation({ summary: 'Obtener actividades no lectivas de una declaración' })
  @ApiResponse({ status: 200, description: 'Actividades obtenidas exitosamente' })
  async getActividadesByDeclaracion(@Param('declaracionId') declaracionId: string) {
    const id = parseInt(declaracionId);
    const actividades = await this.cargaNoLectivaService.getActividadesByDeclaracion(id);
    return {
      data: actividades,
  ...
  async getActividadById(@Param('id') id: string) {
    ...
    const actividad = await this.cargaNoLectivaService.getActividadById(actividadId);
    return {
      data: actividad,
  ...
  async getResumenActividad(@Param('id') id: string) {
    const actividadId = parseInt(id);
    const resumen = await this.cargaNoLectivaService.getResumenActividad(actividadId);
    return {
      data: resumen,
  ...
  @ApiOperation({ summary: 'Obtener resumen completo de carga no lectiva de una declaración' })
  @ApiResponse({ status: 200, description: 'Resumen obtenido exitosamente' })
  ...
  async updateActividad(
    ...
    const actividadId = parseInt(id);
    const actividad = await this.cargaNoLectivaService.updateActividad(actividadId, dto);
    return {
      data: actividad,
  ...
  @ApiResponse({ status: 400, description: 'Datos inválidos o excede horas disponibles' })
  async addHorario(
    @Param('actividadId') actividadId: string,
    @Body() dto: CreateHorarioNoLectivoDto,
  ) {
    const id = parseInt(actividadId);
    const horario = await this.cargaNoLectivaService.addHorarioToActividad(id, dto);
    return {
      data: horario,
  ...
  async updateHorario(
    ...
    const horario = await this.cargaNoLectivaService.updateHorario(horarioId, dto);
    return {
      data: horario,
