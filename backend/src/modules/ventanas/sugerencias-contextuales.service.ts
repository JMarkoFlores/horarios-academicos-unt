...
export class SugestionesContextualesService {
  private diasNombres = ['', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
  ...
  async sugerirAmbientesAlternos(
    ...
    for (const ambiente of ambientesCompatibles) {
      ...
      const disponible = !ocupado || !this.hayTraslape(
        ocupado.hora_inicio,
        ocupado.hora_fin,
        horaInicio,
        horaFin,
      ...
      alternativas.push({
        ...
        disponible,
        razon: disponible ? 'Disponible en este slot' : 'Ocupado en este horario',
      });
    }
  ...
  async sugerirBloquesAlternos(
    ...
    const restricciones = await this.restriccionesService.obtenerRestriccionesPeriodo(periodoId);
    ...
    for (const dia of restricciones.dias_habiles) {
      for (const horario of horasPermitidas) {
        ...
        if (cruceDocente && this.hayTraslape(
    ...
    return alternativas.sort((a, b) => {
      ...
      return preferenciaPriority[a.preferencia] - preferenciaPriority[b.preferencia];
    });
  }
  ...
  private async obtenerAmbienteDisponible(
    ...
    for (const ambiente of curso.ambientes) {
      if (tipoClase === TipoClase.LABORATORIO && ambiente.tipo !== TipoAmbiente.LABORATORIO) {
        continue;
      }
      ...
      if (!ocupado || !this.hayTraslape(ocupado.hora_inicio, ocupado.hora_fin, horaInicio, horaFin)) {
        return ambiente;
      }
    }
  ...
  private async analizarImpactoReasignacion(
    ...
    alternativas: BloqueAlternativo[],
  ): Promise<{ cambios_necesarios: number; grupos_afectados: number[]; docentes_afectados: number[] }> {
  ...
  private hayTraslape(inicio1: string, fin1: string, inicio2: string, fin2: string): boolean {
