  ...
  implements OnModuleDestroy, OnApplicationBootstrap
{
  ...
  async revisarTemporizadores() {
    this.logger.log(
      "[revisarTemporizadores] Revisando temporizadores expirados...",
    );
    ...
    for (const key of keys) {
      ...
      try {
        ...
        if (ahora >= data.timestamp) {
          this.logger.log(
            `[revisarTemporizadores] Temporizador expirado para ventana ${data.ventanaId}. Llamando siguiente docente...`,
          );
        ...
        this.logger.error(
          `[revisarTemporizadores] Error procesando temporizador ${key}: ${error.message}`,
        );
  ...
  async obtenerDocentesParaCategoria(
    proposito: string,
    periodo: string,
    modalidad?: string,
  ): Promise<Docente[]> {
  ...
  async actualizarVentana(
    ventanaId: string,
    dto: UpdateVentanaDto,
  ): Promise<VentanaAtencion> {
    this.logger.log(
      `[actualizarVentana] ID=${ventanaId}, DTO=${JSON.stringify(dto)}`,
    );
    ...
    if (ventana.estado === EstadoVentanaAtencion.EN_CURSO) {
      throw new BadRequestException(
        "❌ No se puede editar una ventana que está en curso",
      );
    }
    if (ventana.estado === EstadoVentanaAtencion.COMPLETADA) {
      throw new BadRequestException(
        "❌ No se puede editar una ventana que ya fue completada",
      );
    }
    ...
    if (dto.fecha || dto.hora_inicio || dto.hora_fin) {
      ...
      const solapamiento = await this.ventanaRepo
        ...
        .andWhere("v.estado != :estado", {
          estado: EstadoVentanaAtencion.CANCELADA,
        })
  ...
  async eliminarVentana(ventanaId: string): Promise<void> {
    ...
    if (ventana.estado === EstadoVentanaAtencion.EN_CURSO) {
      throw new BadRequestException(
        "❌ No se puede eliminar una ventana que está en curso. Finalícela primero.",
      );
    }

    // Eliminar selecciones temporales primero (foreign key constraint)
    await this.ventanaRepo.query(
      'DELETE FROM selecciones_temporales WHERE "ventana_atencion_id" = $1',
      [ventanaId],
    );
    this.logger.log(
      `[eliminarVentana] Selecciones temporales eliminadas para ventana ${ventanaId}`,
    );
  ...
  async crearVentana(dto: CreateVentanaDto): Promise<VentanaAtencion> {
    ...
    if (!dto.fecha || !dto.hora_inicio || !dto.hora_fin) {
      this.logger.error(
        `[crearVentana] ❌ Campos obligatorios faltantes: fecha=${dto.fecha}, hora_inicio=${dto.hora_inicio}, hora_fin=${dto.hora_fin}`,
      );
      throw new BadRequestException(
        "Fecha, hora de inicio y hora fin son obligatorios",
      );
    }
    ...
    if (!horaRegex.test(dto.hora_inicio) || !horaRegex.test(dto.hora_fin)) {
      throw new BadRequestException(
        "Formato de hora inválido. Use HH:MM (24h)",
      );
    }
    ...
    if (minutosFin - minutosInicio < 15) {
      throw new BadRequestException(
        "La ventana debe tener una duración mínima de 15 minutos",
      );
    }
    ...
    if (fechaVentana < hoy) {
      throw new BadRequestException(
        "No se pueden crear ventanas en fechas pasadas",
      );
    }
    ...
    if (
      !dto.saltarValidacionCapacidad &&
      !dto.sinAsignarDocentes &&
      esCategoriaMasiva &&
      dto.periodo
    ) {
      ...
      try {
        docentesEstimados = await this.buscarDocentesElegibles(
          dto.proposito,
          dto.filtro_categorias_docente,
          dto.modalidad,
          dto.periodo,
        );
      } catch (error) {
        this.logger.warn(
          `[crearVentana] No se pudo estimar docentes: ${error.message}`,
        );
        docentesEstimados = [];
      }

      const capacidadMaxima = Math.floor(
        duracionVentanaMinutos / intervaloMinutos,
      );

      if (docentesEstimados.length > capacidadMaxima) {
        this.logger.warn(
          `[crearVentana] ⚠️ Capacidad insuficiente: ${docentesEstimados.length} docentes estimados, capacidad máxima: ${capacidadMaxima}`,
        );
        ...
        const intervaloMinimoNecesario = Math.ceil(
          duracionVentanaMinutos / docentesEstimados.length,
        );
        if (intervaloMinimoNecesario >= 5) {
          sugerencias.push(
            `Reduce el intervalo a ${intervaloMinimoNecesario} minutos (actual: ${intervaloMinutos})`,
          );
        } else {
          sugerencias.push(
            `Reduce el intervalo a 5 minutos (mínimo permitido, actual: ${intervaloMinutos})`,
          );
        }
        ...
        sugerencias.push(
          `Aumenta la duración a ${duracionHoras}h ${duracionMin}min (actual: ${Math.floor(duracionVentanaMinutos / 60)}h ${duracionVentanaMinutos % 60}min)`,
        );

        // Sugerencia 3: Múltiples ventanas
        const ventanasNecesarias = Math.ceil(
          docentesEstimados.length / capacidadMaxima,
        );
    ...
    const solapamiento = await this.ventanaRepo
      ...
      .andWhere(
        "(CAST(:horaInicio AS time) < v.hora_fin AND CAST(:horaFin AS time) > v.hora_inicio)",
        {
    ...
    this.logger.log(
      `Creando ventana: ${dto.fecha} ${dto.hora_inicio}-${dto.hora_fin} (${dto.proposito})`,
    );
    ...
    if (!dto.sinAsignarDocentes && esCategoriaMasiva) {
      ...
      this.logger.log(
        `⚠️ sinAsignarDocentes=true, no se asignan docentes automáticamente`,
      );
    }
    ...
    if (fechaVentana > manana) {
      this.logger.log(
        `Ventana programada para fecha futura (${dto.fecha}). Notificaciones se programarán al iniciar la ventana.`,
      );
    } else {
      this.logger.warn(
        `Ventana programada para fecha cercana (${dto.fecha}). Recordatorio 24h no se enviará.`,
      );
    }
  ...
  async distribuirDocentesEntreVentanas(
    ventanasIds: string[],
    periodo: string,
    proposito: string,
    modalidad?: string,
    ...
    this.logger.log(
      `[distribuirDocentesEntreVentanas] Distribuyendo docentes entre ${ventanasIds.length} ventanas...`,
    );

    // Obtener docentes según el propósito
    const docentes = await this.buscarDocentesPorProposito(
      proposito,
      modalidad,
      periodo,
    );
    this.logger.log(
      `[distribuirDocentesEntreVentanas] ${docentes.length} docentes encontrados`,
    );

    if (docentes.length === 0) {
      throw new BadRequestException(
        "No se encontraron docentes para la categoría especificada",
      );
    }
    ...
    this.logger.log(
      `[distribuirDocentesEntreVentanas] Distribución: ${docentesPorVentanaMin} por ventana, ${docentesExtra} ventanas con 1 extra`,
    );
    ...
    for (let v = 0; v < numVentanas; v++) {
      ...
      this.logger.log(
        `[distribuirDocentesEntreVentanas] Ventana ${v + 1} (${ventanaId}): ${numDocentesEstaVentana} docentes`,
      );
      ...
      if (!ventana) {
        this.logger.warn(
          `[distribuirDocentesEntreVentanas] Ventana ${ventanaId} no encontrada, saltando`,
        );
        continue;
      }
    ...
    this.logger.log(
      `[distribuirDocentesEntreVentanas] ✅ Distribución completada`,
    );
  }
  ...
  async configurarVentanasPeriodo(
    ...
    for (const item of config) {
      ...
      const ventana = this.ventanaRepo.create({
        ...
        hora_fin: this.sumarMinutos(
          item.hora_inicio,
          docentes.length * (item.intervalo_minutos ?? 30),
        ),
      ...
      for (const docente of docentes) {
        ...
        this.logger.log(
          `Programando notificaciones para docente ${docente.id} en ventana ${savedVentana.id}`,
        );
        await this.notificacionesService.enviarRecordatorio24h(
          docente.id,
          savedVentana.id,
        );
        await this.notificacionesService.enviarAlerta15min(
          docente.id,
          savedVentana.id,
        );
      }
  ...
  async iniciarVentana(ventanaId: string) {
    ...
    if (colaExistente.length > 0) {
      this.logger.log(
        `✅ ${colaExistente.length} docente(s) ya asignados a la ventana. Usando cola existente.`,
      );
      ...
      for (const cola of colas) {
        if (cola.docente) {
          try {
            this.logger.log(
              `  → Programando para docente ${cola.docente.id} (${cola.docente.nombres} ${cola.docente.apellidos})...`,
            );
            ...
            this.logger.error(
              `    ❌ Error programando notificaciones: ${err.message}`,
            );
          }
        }
      }
      ...
      this.logger.log(
        `� Resumen notificaciones: ${notificacionesExitosas} exitosas, ${notificacionesFallidas} fallidas`,
      );
    } else {
      // Si no hay docentes asignados, buscar por categoría (solo si es masiva, como DECLARACION)
      if (ventana.proposito === "DECLARACION") {
        this.logger.log(
          `⚠️ No hay docentes asignados. Buscando para propósito ${ventana.proposito}...`,
        );

        // Primero obtener docentes que ya están asignados a otras ventanas del mismo período
        const docentesEnOtrasVentanas = await this.colaRepo
          ...
          .andWhere("v.estado != :estado2", {
            estado2: EstadoVentanaAtencion.COMPLETADA,
          })
          .select("c.docente_id")
          .distinct()
          .getRawMany();

        const docentesIdsExcluidos = docentesEnOtrasVentanas.map(
          (r) => r.docente_id,
        );
        this.logger.log(
          `⚠️ Docentes ya asignados a otras ventanas del período: ${docentesIdsExcluidos.length}`,
        );
        ...
        const docentesDisponibles = docentes.filter(
          (d) => !docentesIdsExcluidos.includes(d.id),
        );

        this.logger.log(
          `✅ ${docentesDisponibles.length} docente(s) disponibles después de filtrar (de ${docentes.length} totales)`,
        );

        if (docentesDisponibles.length === 0) {
          this.logger.warn(
            `⚠️ No hay docentes disponibles para asignar a esta ventana`,
          );
        }
        ...
        this.logger.log(
          `⚠️ No hay docentes asignados y es una ventana ad-hoc (${ventana.proposito}). Iniciando ventana vacía.`,
        );
      }
    }
    ...
    this.gateway.emitirPeriodo(ventana.periodo, "ventana_iniciada", {
      ventanaId,
      estado,
    });
  ...
  async programarNotificacionesVentana(ventanaId: string): Promise<void> {
    ...
    this.logger.log(
      `Programando notificaciones para ${colaDocentes.length} docente(s)`,
    );

    for (const item of colaDocentes) {
      if (item.docente) {
        this.logger.log(
          `Programando para docente ${item.docente.id} (${item.docente.nombres})`,
        );
        await this.notificacionesService.enviarRecordatorio24h(
          item.docente.id,
          ventanaId,
    ...
    this.logger.log(
      `Notificaciones programadas exitosamente para ventana ${ventanaId}`,
    );
  }
  ...
  async llamarSiguiente(ventanaId: string) {
    ...
    if (siguiente) {
      ...
      this.logger.log(
        `[llamarSiguiente] Programando temporizador de ${intervaloMinutos} minutos para ventana ${ventanaId}`,
      );
  ...
  private async buscarDocentesElegibles(
    ...
    this.logger.log(
      `[buscarDocentesElegibles] Buscando docentes: propósito=${proposito}, modalidad=${modalidad}, periodo=${periodo}`,
    );
    ...
    if (
      ["DECLARACION", "SUBSANACION", "CAMBIO", "CONTINGENCIA"].includes(
        proposito,
      )
    ) {
      this.logger.log(
        `[buscarDocentesElegibles] Propósito operativo detectado: ${proposito}`,
      );
      ...
      if (periodo) {
        ...
        if (proposito === "DECLARACION") {
          ...
          proposito === "CAMBIO" ||
          proposito === "CONTINGENCIA"
        ) {
          // Para subsanación/cambio: docentes que SÍ tienen horario
          qb.andWhere(`docente.id IN (${subQueryStr})`, subQb.getParameters());
          this.logger.log(
            `[buscarDocentesElegibles] Filtrando docentes CON horario`,
          );
        }
      } else {
        this.logger.warn(
          `[buscarDocentesElegibles] No se proporcionó período para propósito operativo`,
        );
      }
    }

    if (filtroCategoriasDocente && filtroCategoriasDocente.length > 0) {
      qb.andWhere("docente.categoria IN (:...filtroCategoriasDocente)", {
        filtroCategoriasDocente,
      });
      this.logger.log(
        `[buscarDocentesElegibles] Filtrando por categorías de docente: ${filtroCategoriasDocente.join(", ")}`,
      );
    }
    ...
    if (modalidad) {
      ...
      this.logger.log(
        `[buscarDocentesElegibles] Filtrando por modalidad: ${modalidad}`,
      );
    }
    ...
    this.logger.log(
      `[buscarDocentesElegibles] Encontrados ${docentes.length} docentes`,
    );
    return docentes;
  }
  ...
  private async buscarDocentesPorCategoria(
    ...
    return this.buscarDocentesElegibles(
      "DECLARACION",
      [categoriaDocente],
      modalidad,
      periodo,
  ...
  async sugerirDistribucion(dto: CreateVentanaDto): Promise<{
    ...
    this.logger.log(
      `[sugerirDistribucion] Calculando distribución para: ${JSON.stringify(dto)}`,
    );
    ...
    const docentesEstimados = await this.buscarDocentesElegibles(
      dto.proposito,
      dto.filtro_categorias_docente,
      dto.modalidad,
      dto.periodo,
    ...
    this.logger.log(
      `[sugerirDistribucion] Docentes: ${docentesEstimados.length}, Capacidad: ${capacidadMaxima}, Ventanas: ${ventanasNecesarias}`,
    );
  ...
  async preAsignarDocentes(
    ...
    this.logger.log(
      `[preAsignarDocentes] Ventana ${ventanaId}, docentes: ${docentesIds.join(",")}`,
    );
    ...
    if (ventana.estado === EstadoVentanaAtencion.EN_CURSO) {
      throw new BadRequestException(
        "❌ No se puede pre-asignar docentes a una ventana que está en curso",
      );
    }
    ...
    if (docentes.length !== docentesIds.length) {
      const docentesNoEncontrados = docentesIds.filter(
        (id) => !docentes.find((d) => d.id === id),
      );
      this.logger.warn(
        `⚠️ Docentes no encontrados: ${docentesNoEncontrados.join(",")}`,
      );
    }
    ...
    // Validar capacidad
    const duracionVentanaMinutos = this.calcularDuracionVentana(
      ventana.hora_inicio,
      ventana.hora_fin,
    );
    ...
    if (docentes.length > capacidadMaxima) {
      this.logger.warn(
        `⚠️ Capacidad insuficiente: ${docentes.length} docentes, capacidad: ${capacidadMaxima}`,
      );
      throw new BadRequestException(
    ...
    await this.colaRepo.save(colas);
    this.logger.log(
      `✅ ${docentes.length} docentes pre-asignados a ventana ${ventanaId}`,
    );
    ...
    for (const docente of docentes) {
      try {
        ...
        this.logger.warn(
          `No se pudieron programar notificaciones para docente ${docente.id}: ${err.message}`,
        );
      }
    }
