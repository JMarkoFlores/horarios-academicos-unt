...
export class NotificacionesService {
  ...
  async enviarRecordatorio24h(docenteId: number, ventanaId: number): Promise<void> {
  ...
  async enviarRecordatorio15min(docenteId: number, ventanaId: number): Promise<void> {
    const ventana = await this.ventanaRepo.findOne({ where: { id: ventanaId } });
    if (!ventana) return;
  ...
  async procesarRecordatorio(docenteId: number, ventanaId: number, tipo: string): Promise<void> {
    ...
    this.logger.log(`[EMAIL RECORDATORIO ${tipo}] → ${docente.email}: ${mensaje}`);
  }
  ...
  async upsertPreferencias(docenteId: number, dto: UpdatePreferenciasDto): Promise<PreferenciasNotificacion> {
