import {
  Controller, Get, Param, Query, ParseIntPipe, Res, UseGuards,
} from '@nestjs/common';
...
export class ReportesController {
  ...
  async docentePDF(
    ...
    const buffer = await this.reportesService.generarReporteDocentePDF(id, periodo ?? '');
    res.set({
  ...
  async ambientePDF(
    ...
    const buffer = await this.reportesService.generarReporteAulaPDF(id, periodo ?? '');
    res.set({
      'Content-Type': 'application/pdf',
  ...
  @ApiQuery({ name: 'periodo', required: true, example: '2026-I' })
  async operacionalPDF(@Query('periodo') periodo: string, @Res() res: Response) {
    const buffer = await this.reportesService.generarReporteOperacionalPDF(periodo ?? '');
    res.set({
      'Content-Type': 'application/pdf',
