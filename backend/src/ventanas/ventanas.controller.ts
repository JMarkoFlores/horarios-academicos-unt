...
export class VentanasController {
  ...
  @ApiOperation({ summary: 'Iniciar ventana y cargar cola de docentes por jerarquía' })
  ...
  async siguiente(@Param('id', ParseIntPipe) id: number) {
    ...
    return { data: result, message: result ? 'Siguiente docente llamado' : 'Cola finalizada' };
  }
  ...
  async confirmar(
    ...
    const result = await this.ventanasService.confirmarSeleccion(id, dto.docente_id);
