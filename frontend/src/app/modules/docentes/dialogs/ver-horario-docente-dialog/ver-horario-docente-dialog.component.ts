...
export class VerHorarioDocenteDialogComponent implements OnInit, OnDestroy {
        ...
        next: (r) => {
          ...
          if (almuerzo?.valor?.hora_inicio && almuerzo?.valor?.hora_fin) {
            this.almuerzoInicio = parseInt(
              almuerzo.valor.hora_inicio.split(':')[0], 10,
            );
            this.almuerzoFin = parseInt(
              almuerzo.valor.hora_fin.split(':')[0], 10,
            );
            this._buildGrid();
          }
        },
  ...
  private _buildGrid(): void {
    ...
    for (const dia of this.diasNum) {
      for (const hora of this.horas) {
        ...
        const asig = this.asignaciones.find(
  ...
  getCell(dia: number, hora: number): CeldaHorario {
    return this._gridCache.get(`${dia}_${hora}`) ?? { asig: null, rowspan: 1, skip: false, esAlmuerzo: false };
  }
  ...
  cls(dia: number, hora: number): string {
    ...
    return cell.asig.tipo_clase === 'LABORATORIO' ? 'celda-lab' : 'celda-teoria';
  }
  ...
  descargarPdf(): void {
    ...
    try {
        ...
        for (const dia of this.diasNum) {
          ...
          if (asig) {
            ...
            cols.push({
              ...
              tipo:  asig.tipo_clase === 'LABORATORIO' ? 'LABORATORIO' : 'TEORIA',
      ...
      const head = [['Hora', 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes']];
      ...
      doc.text(`Almuerzo (${this.fmtHStr(almuerzIni)}-${this.fmtHStr(almuerzFin)})`, 76, pieY + 5);
